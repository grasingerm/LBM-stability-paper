...
Yield stress flow has been found to be relevant in many applications, such as flow of pastes, paints, muds, molten plastics and metals, and in some cases, blood~\cite{wang2011lattice}.
...
\Fref{fig:feq-vs-f_mrt} compares particle distribution, $f_i$, profiles to their respective quasiequilibrium profiles, $f_i^{eq}$ for the MRT model.
...
The 2-2.5 times increase in computing time resulted in the BGK collision schemes on the finer lattice being comparable in computational cost to the MRT collision scheme on the coarser lattice.
...
The MRT collision operator, even on the coarser lattice, was more accurate (for the case of $Bn = 8.52$) than all of the BGK collision schemes on the finer lattice.
...
\begin{equation} \label{eq:analytical-power-law}
  u_x(y) = \frac{n}{n+1}\left(-\frac{1}{2k}\pgrad\right)^{1/n}\left[h^{\frac{n+1}{n}} - |y|^{\frac{n+1}{n}}\right].
\end{equation}
...
The flow consistency index was held constant ($k = 0.2$.), while the pressure gradient was varied in order to match the Reynold's numbers in \Fref{tab:poise-power-law}.
The relative $L_2$ error, relative $L_{\infty}$ error, and computation time for each simulation are presented in \Fref{tab:poise-power-law-2}.
...
\begin{equation} \label{eq:gradu-power-law}
  \frac{\partial u_x}{\partial y} = \left( -\frac{1}{2k} \pgrad \right)^{1/n} \left( -y |y|^{\frac{1}{n} - 1} \right).
\end{equation}
