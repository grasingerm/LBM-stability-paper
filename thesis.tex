...
For example, ~\citet{tang2011bingham,chai2011multiple,fallah2012multiple,chen2014simulations,vikhansky2008lattice,wang2008lattice} developed LBM models for simulating yield stress flow.
...
\citet{wang2011lattice} developed another constant-collision frequency LBM scheme for non-Newtonian flow by splitting the effects of constitutive relationship into Newtonian and non-Newtonian parts.
...
A disadvantage of \eqref{eq:dissipation-critera} is that if $\overline{\Delta S}$ and $n_\sigma$ are both small then it is possible for dissipation to be added when $\Delta S(\pos, t)$ is low and artifical dissipation is unnecessary.
By required that both \eqref{eq:ds-theshold} and \eqref{eq:dissipation-criteria} be met before artifical dissipation is added, there is the potential for \eqref{eq:ds-threshold} and \eqref{eq:dissipation-criteria} to compensate for the each other's disadvantage.
...
\begin{equation}
  \frac{\sum_{\pos \in \domain} \epsilon(\pos, 16000)}{\sum_{\pos \in \domain} 1.0}.
\end{equation}
...
An LBM model for simulating non-Newtonnian and free-surface flow to represent cementing dry annuli in oil and gas wellbores was presented.
...
In addition, possible future work could include investigating other physical problems such as planar sudden expansion flow~\cite{tang2011binghami,chai2011multiple,chen2014simulations}, reentrant corner flow~\cite{gabbanelli2005lattice},
