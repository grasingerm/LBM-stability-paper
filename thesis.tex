...
Determining the apparent viscosity and strain-rate of a flow will often require an iterative solution, a general Picard-type algorithm is:
...
\frac{\epsilon(\pos, t) + \epsilon(\pos + \pvel_i \Delta t, t}{2} (f_\opidx(\pos + \pvel_i, t) - f_i(\pos, t)), & \pos \in \iset \text{ and } \pos + \pvel_i \Delta t \in \iset \\
...
Moreover, when mass redistribution inevitably occurs, it is first attempted to be redistributed and weighted in the direction in which the interface is propagating (e.g. line \algref{algo:update-cell-states}{line:redist-dop-f} or line \algref{line:redist-dop-g}.
If there are no interface cells in the direction of interface propagation, it is instead redistributed to neighboring interface cells (e.g. line \algref{line:redist-neighbors-f} or line \algref{line:redist-neighbors-g}).
If there is no neighboring cells which are interface cells, the excess mass is added onto the global excess mass, $M_{gex}$, which is eventually redistributed among all of the interface cells (e.g. line \algref{line:final-redist}).
...
The figure on the right is originally from~\cite{cementing2015web}}
