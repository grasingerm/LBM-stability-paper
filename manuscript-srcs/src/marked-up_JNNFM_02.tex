...
This work examines various collision operators and methods for stability enhancement for their suitability for simulating non-Newtonian fluid flows in terms of \DIFaddbegin \DIFadd{their }\DIFaddend accuracy, numerical stability and computational efficiency.
...
It was also shown that artificial dissipation can be an effective technique for enhancing \DIFaddbegin \DIFadd{the }\DIFaddend numerical stability of non-Newtonian, lid-driven cavity flow \DIFaddbegin \DIFadd{simulations}\DIFaddend , and is particularly effective for efficiently and accurately simulating the flow of shear-thinning fluids.

\section{Introduction}
%DIF < MG: this intro will potentially be long. Should it be broken up into subsections (as it is now)?
...
Yield stress flow has been found to be relevant in many \DIFdelbegin \DIFdel{applictions}\DIFdelend \DIFaddbegin \DIFadd{applications}\DIFaddend , such as \DIFaddbegin \DIFadd{the }\DIFaddend flow of pastes, paints, muds, molten plastics and metals, and in some cases, blood~\cite{wang2011lattice}.
%DIF < MG: citation is for blood as non-Newtonian

Power-law behavior \DIFdelbegin \DIFdel{, is more commonly }\DIFdelend \DIFaddbegin \DIFadd{is also }\DIFaddend known as \emph{shear-thinning} \DIFdelbegin \DIFdel{, }\DIFdelend when the apparent viscosity decreases with increasing strain-rate, or \emph{shear-thickening} \DIFdelbegin \DIFdel{, }\DIFdelend when the apparent viscosity increases with increasing strain-rate.
...
%DIF < MG: probably include a discussion of the work done by Conrad et. al. 2015 here
\DIFdelbegin \DIFdel{LBM does}\DIFdelend \DIFaddbegin \DIFadd{LBM does, }\DIFaddend however, have its drawbacks.
...
%DIF < MG: are all relevant sources cited?
...
%DIF < MG: be more precise than 'fluctuations'?
...
The equilibrium distribution function can be derived for \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend specific constitutive relationship of interest using the Chapman-Enskog multiscale expansion.
...
\cite{wang2011lattice} developed another \DIFdelbegin \DIFdel{constant-collision }\DIFdelend \DIFaddbegin \DIFadd{constant collision }\DIFaddend frequency LBM scheme for non-Newtonian flow by splitting the effects of constitutive relationship into Newtonian and non-Newtonian parts.
...
Although the \DIFdelbegin \DIFdel{constant-collision }\DIFdelend \DIFaddbegin \DIFadd{constant collision }\DIFaddend frequency strategies present interesting alternatives, the variable collision frequency scheme is used in the present study because of its simplicity and generality, such as the fact that a variable collision frequency scheme can be fit to any constitutive relationship without performing Chapman-Enskog multiscale expansion.

%The Bhatnagar, Gross, Krook (BGK) collision operator~\cite{bhatnagar1954model}.
The Multiple-relaxation-time (MRT) collision operator ~\cite{d1994generalized} is another approach that has been developed to improve \DIFdelbegin \DIFdel{upon }\DIFdelend the stability of LBM.
...
%DIF < MG: is the beginning of this methods section too much fluff now? Would our audiance consider it fluff?
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Lattice Boltzmann Method for Simulation of Non-Newtonian Fluids} \label{sec:LBM}
...
The length scale of LBM is unique in contrast to most common numerical methods \DIFdelbegin \DIFdel{, }\DIFdelend and is referred to as the mesoscale.
...
%DIF <  MG: do we even need this citation?
...
%DIF <  MG: is this true of all collision operators? or just BGK?
...
The BGK \DIFdelbegin \DIFdel{collison }\DIFdelend \DIFaddbegin \DIFadd{collision }\DIFaddend operator for the $i^{th}$ discrete velocity is expressed as:
...
For example, in low viscosity fluids\DIFaddbegin \DIFadd{, }\DIFaddend the BGK operator results in an overrelaxation of particle distributions toward quasiequilibrium.
...
Overrelaxation will occur if $\nu \rightarrow 0$ because \DIFdelbegin \DIFdel{consquently }\DIFdelend \DIFaddbegin \DIFadd{consequently }\DIFaddend $\omega \rightarrow 2$ and \eqref{eq:bgk} results in $f_i(\pos + \pvel_i \Delta t,t + \Delta t)$ still being ``far'' from $f^{eq}_i(\pos + \pvel_i \Delta t,t + \Delta t)$ but on the ``other side''.
...
Due to the instabilities associated with the collision frequency being too high (e.g. approaching 2) or too low (e.g. approaching 0), a natural, \DIFdelbegin \DIFdel{albiet }\DIFdelend \DIFaddbegin \DIFadd{albeit }\DIFaddend nonphysical, approach to using the BGK collision operator for non-Newtonian flow is to simply put bounds on the values in which the collision frequency may attain (such as in~\citet{svec2011flow,svec2012free,gabbanelli2005lattice}).
...
The idea of artificial dissipation is to increase numerical stability \DIFdelbegin \DIFdel{, }\DIFdelend while sacrificing some physical accuracy.
...
A metric that has been developed and used successfully for determining when artificial dissipation should be introduced at a lattice site is the \DIFdelbegin \DIFdel{so called }\DIFdelend \DIFaddbegin \DIFadd{so-called }\DIFaddend relative nonequilibrium entropy~\cite{gorban2014enhancement,brownlee2006stabilization,brownlee2007stability,brownlee2008nonequilibrium,packwood2009entropy}.
...
\noindent A more computationally efficient approximation of $\Delta S$ can be achieved by instead using the \DIFdelbegin \DIFdel{second order }\DIFdelend \DIFaddbegin \DIFadd{second-order }\DIFaddend Taylor expansion of \eqref{eq:rel-noneq-entropy}:
...
\noindent where $\overline{\Delta S}$ and $\sigma_{\Delta S}$ are the mean and standard deviation of $\Delta S$, respectively--both are calculated using values over the domain for the current \DIFdelbegin \DIFdel{timestep--and }\DIFdelend \DIFaddbegin \DIFadd{time step--and }\DIFaddend $n_\sigma$ is the number of standard deviations greater than $\overline{\Delta S}$ that $\Delta S$ must be before dissipation is added.
...
A disadvantage of \eqref{eq:dissipation-criteria} is that if $\overline{\Delta S}$ and $n_\sigma$ are both small then it is possible for dissipation to be added when $\Delta S(\pos, t)$ is low and \DIFdelbegin \DIFdel{artifical }\DIFdelend \DIFaddbegin \DIFadd{artificial }\DIFaddend dissipation is unnecessary.
By requiring that both \eqref{eq:ds-threshold} and \eqref{eq:dissipation-criteria} be met before \DIFdelbegin \DIFdel{artifical }\DIFdelend \DIFaddbegin \DIFadd{artificial }\DIFaddend dissipation is added, there is the potential for \eqref{eq:ds-threshold} and \eqref{eq:dissipation-criteria} to compensate for the each other's disadvantage.
...
One method of adding dissipation is the so-called Ehrenfests' regularization~\cite{brownlee2006stabilization} \DIFdelbegin \DIFdel{, }\DIFdelend and involves setting a lattice site that is chosen for artificial dissipation to its quasiequilibrium state.
Although this achieves the desired result, namely \DIFdelbegin \DIFdel{damping }\DIFdelend \DIFaddbegin \DIFadd{dampening }\DIFaddend out large nonequilibrium fluctuations, it does so in a way that is not smooth or gradual, but sharp.
...
In LBM this is a desirable way to introduce dissipation because it has the potential to reduce \DIFdelbegin \DIFdel{high frequency }\DIFdelend \DIFaddbegin \DIFadd{high-frequency }\DIFaddend nonequilibrium fluctuations that might lead to numerical instability while retaining the lower frequency dynamics.
...
%DIF < MG: show equation for feq that maximizes entropy and give a brief word why we should care
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Boundary Conditions and Applied Forces} \label{sec:bcs}
...
The particle distributions that are missing after the streaming step are solved for by assuming a \DIFdelbegin \DIFdel{bounceback }\DIFdelend \DIFaddbegin \DIFadd{bounce back }\DIFaddend of the nonequilibrium distribution in the direction normal to the boundary; e.g., for a south inlet or outlet, $f_2 - f_2^{eq} = f_4 - f_4^{eq}$.
...
Both the collision frequency and the $s_7$ and $s_8$ relaxation rates are dependent on the apparent \DIFdelbegin \DIFdel{visocity}\DIFdelend \DIFaddbegin \DIFadd{viscosity}\DIFaddend , $\mu_{app}$, through \eqref{eq:colfreq-non-newtonian} and \eqref{eq:mrt-s7}, respectively.
...
A Bingham plastic does not flow (i.e. the strain-rate is zero) when the shear stress is below the yield stress \DIFdelbegin \DIFdel{, }\DIFdelend and behaves in an almost Newtonian manner when the shear stress is above the yield stress.
...
\subsection{\DIFaddbegin \DIFadd{Bingham Plastic, }\DIFaddend Poiseuille Flow} \DIFaddbegin \label{sec:poise-bing}
\DIFaddend 

Poiseuille flow is a useful benchmark because \DIFdelbegin \DIFdel{analytical solutions exist for both Bingham plastic and power-law fluids}\DIFdelend \DIFaddbegin \DIFadd{an analytical solution exists for Bingham plastic flow}\DIFaddend .
...
Unless otherwise stated, all of the Poiseuille flow simulations were computed on a $32 \times 64$ lattice for 25000 \DIFdelbegin \DIFdel{timesteps}\DIFdelend \DIFaddbegin \DIFadd{time steps}\DIFaddend .
The lattice size was chosen to be sufficiently fine for accuracy and the number of time steps \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend set high enough to ensure a \DIFdelbegin \DIFdel{steady-state }\DIFdelend \DIFaddbegin \DIFadd{steady state }\DIFaddend would be reached.
In reference to computational time, each of the simulations in this section \DIFdelbegin \DIFdel{were }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend run on a single core of an Intel I7-860 Quad-Core 2.80GHz processor.
\DIFdelbegin \DIFdel{For the MRT relaxation matrix the free parameters were set to $s_1 = s_2 = s_4 = s_6 = 1.1$, which follows the recommendation of~\mbox{%DIFAUXCMD
\cite{lallemand2000theory} }%DIFAUXCMD
for reasons of stability, and has been successfully used to simulate non-Newtonian flow in the past~\mbox{%DIFAUXCMD
\cite{fallah2012multiple,grasinger2015simulation}}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsubsection{\DIFdel{Bingham Plastic Fluids}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{sec:poise-bing}%%%
%DIFDELCMD < %%%
\DIFdelend 
...
The yield stress was varied between four different values $\tau_y = \begin{bmatrix}4.0,&8.0,&12.0,&16.0\end{bmatrix} \times 10^{-5}$, and \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend different LBM schemes were used: (1) BGK with $m = 10^5$, (2) BGK with $m = 10^8$, (3) BGK with $m = 10^8$ and the median filter, \DIFdelbegin \DIFdel{and }\DIFdelend (4) \DIFdelbegin \DIFdel{MRT }\DIFdelend \DIFaddbegin \DIFadd{MRT-1 with $m = 10^8$, and (5) MRT-2 }\DIFaddend with $m = 10^8$.
\DIFdelbegin \DIFdel{Recall }\DIFdelend \DIFaddbegin \DIFadd{The MRT-1 and MRT-2 schemes differ in their choice of free parameters for the relaxation matrix.
For the MRT-1 relaxation matrix, the free parameters were set to $s_1 = s_2 = s_4 = s_6 = 1.1$, which follows the recommendation of~\mbox{%DIFAUXCMD
\cite{lallemand2000theory} }%DIFAUXCMD
for reasons of stability, and has been successfully used to simulate non-Newtonian flow in the past~\mbox{%DIFAUXCMD
\cite{fallah2012multiple,grasinger2015simulation}}%DIFAUXCMD
. 
For the MRT-2 relaxation matrix, the free parameters were set to $s_1 = 1.1$, $s_2 = 1.0$, and $s_4 = s_6 = 1.2$, as these values have been successfully applied to simulating lid-driven cavity flow of non-Newtonian fluids in the past~\mbox{%DIFAUXCMD
\cite{chen2014simulations,li2014simulation}}%DIFAUXCMD
.
Additionally, in regards to the LBM schemes, recall }\DIFaddend that $m$ is the stress growth exponent for the Papanastasiou approximation, and the higher $m$ is the closer \DIFaddbegin \DIFadd{the approximation is }\DIFaddend to the true Bingham plastic behavior.
...
%DIF <  MG: it might be better to normalize the time with respect to the fastest, BGK with no filtering
\DIFdelbegin %DIFDELCMD < 
...
\\
\DIFdelbeginFL \DIFdelFL{MRT }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MRT-1 }\DIFaddendFL & No & $10^8$ & 4.0 & 6.05 & 0.68 & 0.0013 & 0.0013 & 5914 \\
& & & 8.0 & 4.42 & 1.85 & 0.0018 & 0.0018 & 7908 \\
& & & 12.0 & 3.04 & 4.04 & 0.0026 & 0.0026 & 7160 \\
& & & 16.0 & 1.92 & 8.52 & 0.0041 & 0.0041 & 5559 \\
\\
\DIFaddbeginFL \DIFaddFL{MRT-2 }& \DIFaddFL{No }& \DIFaddFL{$10^8$ }& \DIFaddFL{4.0 }& \DIFaddFL{6.05 }& \DIFaddFL{0.68 }& \DIFaddFL{0.0012 }& \DIFaddFL{0.0012 }& \DIFaddFL{5284 }\\
& & & \DIFaddFL{8.0 }& \DIFaddFL{4.42 }& \DIFaddFL{1.85 }& \DIFaddFL{0.0017 }& \DIFaddFL{0.0017 }& \DIFaddFL{5290 }\\
& & & \DIFaddFL{12.0 }& \DIFaddFL{3.04 }& \DIFaddFL{4.04 }& \DIFaddFL{0.0024 }& \DIFaddFL{0.0024 }& \DIFaddFL{5244 }\\
& & & \DIFaddFL{16.0 }& \DIFaddFL{1.92 }& \DIFaddFL{8.52 }& \DIFaddFL{0.0038 }& \DIFaddFL{0.0038 }& \DIFaddFL{4532 }\\
\\
\DIFaddendFL \label{tab:poise-bing}
\end{tabulary}
\end{table}
...
Upon inspection of \Fref{tab:poise-bing}, it does not appear that entropic median filtering helped to mitigate errors that \DIFdelbegin \DIFdel{occured }\DIFdelend \DIFaddbegin \DIFadd{occurred }\DIFaddend as a result of using $m = 10^8$ for Bingham plastic Poiseuille flow.
...
In order to ensure that this \DIFdelbegin \DIFdel{phenomena }\DIFdelend \DIFaddbegin \DIFadd{phenomenon }\DIFaddend was a side effect of all entropic filtering, and not just entropic median filtering with $\theta = 1.0 \times 10^{-6}$ and $n_s = 2.7$, optimization was used to find the value of $\theta$ that minimized error for both median filtering and Ehrenfests' regularization.
...
The results of the optimization show that entropic filtering, at best\DIFaddbegin \DIFadd{, }\DIFaddend did not affect the accuracy of LBM\DIFdelbegin \DIFdel{, and often adversly }\DIFdelend \DIFaddbegin \DIFadd{; and often adversely }\DIFaddend affected the accuracy of LBM in approximating Bingham plastic Poiseuille flow, regardless of whether median filtering or Ehrenfests' regularization was used, and regardless of what the $\Delta S$ threshold, $\theta$, was set to.
...
On average, the LBM model using the \DIFdelbegin \DIFdel{MRT }\DIFdelend \DIFaddbegin \DIFadd{MRT-1 }\DIFaddend collision operator took 2.9 times more computing time than the BGK collision operator with $m = 10^5$\DIFaddbegin \DIFadd{; and the model using MRT-2 took 2.1 times more computing time than the BGK collision operator with $m = 10^5$}\DIFaddend .
However, the increased computing time can be justified for the MRT collision operator because the MRT \DIFdelbegin \DIFdel{solution }\DIFdelend \DIFaddbegin \DIFadd{solutions }\DIFaddend did not suffer from the same level of nonphysical oscillations as the BGK collision operator when using the more accurate approximation of the Bingham plastic model, i.e. when $m = 10^8$. 
The MRT collision \DIFdelbegin \DIFdel{operator was therefore}\DIFdelend \DIFaddbegin \DIFadd{schemes were, therefore, }\DIFaddend the most accurate \DIFdelbegin \DIFdel{solution }\DIFdelend \DIFaddbegin \DIFadd{solutions }\DIFaddend for all cases\DIFaddbegin \DIFadd{; and because the difference in accuracy between the MRT-1 and MRT-2 collision schemes was negligible, the remainder of the discussion in this section will focus on comparing the BGK collision schemes and the MRT-1 collision scheme}\DIFaddend .
\Fref{fig:feq-vs-f_mrt} compares particle distribution, $f_i$, profiles to their respective \DIFdelbegin \DIFdel{quasiequilbirium }\DIFdelend \DIFaddbegin \DIFadd{quasiequilibrium }\DIFaddend profiles, $f_i^{eq}$ for the \DIFdelbegin \DIFdel{MRT }\DIFdelend \DIFaddbegin \DIFadd{MRT-1 }\DIFaddend model.
...
\begin{figure}
    ...
    \end{tabulary}
    \caption{Particle distributions in the 5 direction (left) and 8 direction (right) compared to their respective quasiequilibriums for the \DIFdelbeginFL \DIFdelFL{MRT }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MRT-1 }\DIFaddendFL with $m = 10^8$.}
...
The values measured for the BGK with $m = 10^8$, with and without median filtering, were significantly higher than for either the \DIFdelbegin \DIFdel{MRT }\DIFdelend \DIFaddbegin \DIFadd{MRT-1 }\DIFaddend or BGK with $m = 10^5$ collision schemes, which suggests that the nonphysical oscillations the BGK with $m = 10^8$ displayed was in part due to the $\epsilon$ moment.
...
It is likely that the nonequilibrium $\epsilon$ and $q_x$ moments cause nonphysical oscillations in particle distributions, $f_i$, with a cumulative effect in time (i.e. \DIFdelbegin \DIFdel{osciallations }\DIFdelend \DIFaddbegin \DIFadd{oscillations }\DIFaddend build in amplitude with time).
...
\begin{figure}
    ...
    \caption{Cumulative relative $L_2$ norm, $\epsilon^{neq}$ with time. Oscillations can have a positive effect on each other\DIFdelbeginFL \DIFdelFL{, building up}\DIFdelendFL . The cumulative relative $L_2$, $\epsilon^{neq}$ is a measure of how much oscillations due to the $\epsilon$ moment may have been building in time.}
    \label{fig:epsilon-cumulative}
\end{figure}

\begin{figure}
    \includegraphics[width=\linewidth]{figs/poise-bingham/qx_cumulative}
    \caption{Cumulative relative $L_2$ norm, $\mathbf{q_x^{neq}}$ with time. Oscillations can have a positive effect on each other\DIFdelbeginFL \DIFdelFL{, building up}\DIFdelendFL . The cumulative relative $L_2$, $\mathbf{q_x^{neq}}$ is a measure of how much oscillations due to the $q_x$ moment may have been building in time.}
    \label{fig:qx-cumulative}
\end{figure}

\DIFdelbegin \subsubsection{\DIFdel{Power-law Fluids}} %DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{sec:poise-powerlaw}%%%
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The simulations of the Poiseuille flow with the power-law constitutive relationship were carried out with a pressure gradient of $\pgrad = 1.0 \times 10^{-5}$, and a flow consistency index of $k = 0.2$.
...
The relative $L_2$ and relative $L_{\infty}$ errors with respect to }\DIFdelend \DIFaddbegin \DIFadd{Lastly, simulations with }\DIFaddend the \DIFdelbegin \DIFdel{analytical solution were computed for each simulation.
...
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eq:analytical-power-law}%%%
...
%DIFDELCMD < %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{same collision schemes, Reynold's numbers, and Bingham numbers were run on $128 \times 32$ lattices in order to investigate the effect of grid resolution.
The plastic viscosity was held constant ($\mu_p = 0.2$.), while the yield stress and pressure gradient were varied in order to match the Reynold's numbers and Bingham numbers in }\Fref{tab:poise-bing}\DIFadd{.
The }\DIFaddend relative $L_2$ error, relative $L_{\infty}$ error, and computation time for each simulation are presented in \DIFdelbegin %DIFDELCMD < \Fref{tab:poise-power-law}%%%
\DIFdel{.
The Reynold's number was computed by $Re = \frac{\rho U^{2-n} H^n}{k}$.
Although for $n = 0.5, 0.75, 1.25$ the MRT collision operator was more accurate and required more computation time than }\DIFdelend \DIFaddbegin \Fref{tab:poise-bing-2}\DIFadd{.
}

\DIFadd{As would be expected from doubling the number of lattice sites, the cost for each corresponding combination of collision scheme, $Re$, and $Bn$ increased by a factor of approximately 2--2.5.
The 2--2.5 times increase in computing time resulted in }\DIFaddend the BGK collision \DIFdelbegin \DIFdel{operator, for $n = 1.5$ the BGK operator with median filtering required the least computation time and was the most accurate with respect to the analytical solution.
...
Note that none of the }\DIFdelend \DIFaddbegin \DIFadd{schemes on the finer lattice being comparable in computational cost to the MRT collision schemes on the coarser lattice.
In regards to error, for the larger Bingham number flows ($Bn = 4.04, 8.52$) both the $L_2$ and $L_\infty$ errors of the BGK }\DIFaddend collision \DIFdelbegin \DIFdel{schemes were sufficiently accurate (error greater than 15\%) }\DIFdelend \DIFaddbegin \DIFadd{schemes were reduced in general, sometimes by as much as a factor of 4.
...
The MRT collision schemes, even on the coarser lattice, were more accurate (}\DIFaddend for the case of \DIFdelbegin \DIFdel{$n = 0.5$, and to improve this accuracy it would be necessary to use a different simulation Mach number, a finer grid, and/or a smaller pressure gradient}\DIFdelend \DIFaddbegin \DIFadd{$Bn = 8.52$) than all of the BGK collision schemes on the finer lattice.
Again, the difference in the accuracies between the MRT-1 and MRT-2 collision schemes appears to be negligible.
The difference in computational cost is also low, though the MRT-2 collision scheme took 4\% less computing time on average}\DIFaddend .
...
\DIFdelend \begin{table}
	\centering
	\caption{\DIFdelbeginFL \DIFdelFL{Power-law }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bingham plastic }\DIFaddendFL Poiseuille flow\DIFaddbeginFL \DIFaddFL{, $128 \times 32$ lattice}\DIFaddendFL }
	\vspace{0.5cm}
	\DIFdelbeginFL %DIFDELCMD < \begin{tabulary}{\linewidth}{l l r r r r r}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabulary}{\linewidth}{l l r r r r r r r r}
		\DIFaddendFL \pbox{20cm}{Collision \\ Operator} & \pbox{20cm}{Median \\ Filter} & \DIFdelbeginFL \DIFdelFL{$n$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$m$ }& \pbox{20cm}{$\tau_y$ \\ $(\times 10^{-5})$} \DIFaddendFL & $Re$ & \DIFaddbeginFL \DIFaddFL{$Bn$ }& \DIFaddendFL $L_2$ & $L_\infty$ & \pbox{20cm}{Time \\ (sec)} \\
		\hline \\
		BGK & No & \DIFdelbeginFL \DIFdelFL{0.50 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$10^5$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0007 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{28.06 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.05 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{30.37 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.68 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{5232 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0419 }& \DIFaddFL{0.0450 }& \DIFaddFL{4951 }\DIFaddendFL \\
		              & & \DIFdelbeginFL \DIFdelFL{0.75 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{0.9125 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0328 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.42 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0569 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.85 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2399 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0300 }& \DIFaddFL{0.0370 }& \DIFaddFL{5209 }\DIFaddendFL \\
	                  & & \DIFdelbeginFL \DIFdelFL{1.25 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{423.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0051 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0055 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2311 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0206 }& \DIFaddFL{0.0260 }& \DIFaddFL{5296 }\DIFaddendFL \\
		              & & \DIFdelbeginFL \DIFdelFL{1.50 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{2213 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.92 }& \DIFaddFL{8.52 }& \DIFaddFL{0.0649 }& \DIFaddFL{0.1016 }& \DIFaddFL{5304 }\\
		\\
		\DIFaddFL{BGK }& \DIFaddFL{No }& \DIFaddFL{$10^8$ }\DIFaddendFL & 1.0 & \DIFdelbeginFL \DIFdelFL{3622 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.05 }& \DIFaddFL{0.68 }& \DIFaddFL{0.0426 }& \DIFaddFL{0.0470 }& \DIFaddFL{6484 }\\
		              & & & \DIFaddFL{2.0 }& \DIFaddFL{4.42 }& \DIFaddFL{1.85 }& \DIFaddFL{0.0306 }& \DIFaddFL{0.0394 }& \DIFaddFL{7111 }\\
		              & & & \DIFaddFL{3.0 }& \DIFaddFL{3.04 }& \DIFaddFL{4.04 }& \DIFaddFL{0.0230 }& \DIFaddFL{0.0438 }& \DIFaddFL{7595 }\\
		              & & & \DIFaddFL{4.0 }& \DIFaddFL{1.92 }& \DIFaddFL{8.52 }& \DIFaddFL{0.0963 }& \DIFaddFL{0.1659 }& \DIFaddFL{7791 }\DIFaddendFL \\
		\\
		BGK & Yes & \DIFdelbeginFL \DIFdelFL{0.50 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$10^8$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0007 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{26.72 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.05 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{28.78 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.68 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{5347 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0427 }& \DIFaddFL{0.0468 }& \DIFaddFL{5576 }\DIFaddendFL \\
		               & & \DIFdelbeginFL \DIFdelFL{0.75 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{0.9125 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0328 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.42 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0569 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.85 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2385 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0309 }& \DIFaddFL{0.0397 }& \DIFaddFL{7512 }\DIFaddendFL \\
		               & & \DIFdelbeginFL \DIFdelFL{1.25 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{423.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0051 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0055 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2275 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0236 }& \DIFaddFL{0.0443 }& \DIFaddFL{7856 }\DIFaddendFL \\
		               & & \DIFdelbeginFL \DIFdelFL{1.50 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{2213 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0570 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.92 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0600 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{8.52 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2357 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0934 }& \DIFaddFL{0.1624 }& \DIFaddFL{7848 }\DIFaddendFL \\
		\\
		\DIFdelbeginFL \DIFdelFL{MRT }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MRT-1 }\DIFaddendFL & No & \DIFdelbeginFL \DIFdelFL{0.50 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$10^8$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0007 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.1758 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.05 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.1401 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.68 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4906 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0452 }& \DIFaddFL{0.0484 }& \DIFaddFL{13837 }\DIFaddendFL \\
		              & & \DIFdelbeginFL \DIFdelFL{0.75 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{0.9125 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0058 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.42 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0045 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.85 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4880 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0404 }& \DIFaddFL{0.0435 }& \DIFaddFL{16515 }\DIFaddendFL \\
                      & & \DIFdelbeginFL \DIFdelFL{1.25 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{423.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0051 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0055 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4863 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0316 }& \DIFaddFL{0.0338 }& \DIFaddFL{17142 }\DIFaddendFL \\
                      & & \DIFdelbeginFL \DIFdelFL{1.50 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{2213 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.92 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{8.52 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{15992 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0197 }& \DIFaddFL{0.0210 }& \DIFaddFL{17853 }\DIFaddendFL \\
		\\
		\DIFdelbeginFL \DIFdelFL{BGK-BRT }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MRT-2 }\DIFaddendFL & No & \DIFdelbeginFL \DIFdelFL{0.50 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$10^8$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0007 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{11.73 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.05 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{12.56 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.68 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{4450 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0453 }& \DIFaddFL{0.0485 }& \DIFaddFL{14030 }\DIFaddendFL \\
		& & \DIFdelbeginFL \DIFdelFL{0.75 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{0.9125 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0320 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.42 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0475 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.85 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1805 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0404 }& \DIFaddFL{0.0435 }& \DIFaddFL{15290 }\DIFaddendFL \\
		& & \DIFdelbeginFL \DIFdelFL{1.25 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{423.2 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0051 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{0.0055 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.04 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1905 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0316 }& \DIFaddFL{0.0338 }& \DIFaddFL{16328 }\DIFaddendFL \\
		& & \DIFdelbeginFL \DIFdelFL{1.50 }\DIFdelendFL & \DIFdelbeginFL \DIFdelFL{2213 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{4.0 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{1.92 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{1.0 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{8.52 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{8194 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{0.0198 }& \DIFaddFL{0.0210 }& \DIFaddFL{16816 }\DIFaddendFL \\
		\\
		\DIFdelbeginFL %DIFDELCMD < %DIFDELCMD < \label{tab:poise-power-law}%%%
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{tab:poise-bing-2}
	\DIFaddendFL \end{tabulary}
\end{table}
...
Taking the partial derivative with respect to $y$ of the analytical solution for the flow velocity of a Bingham plastic, Poiseuille flow (Equation }%DIFDELCMD < \eqref{eq:analytical-bingham}%%%
\DIFdel{) results in:
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eq:gradu-bingham}%%%
...
%DIFAUXCMD
\DIFdel{and taking the partial derivative with respect to $y$ of the analytical solution for the flow velocity of a power-law fluid, Poiseuille flow (Equation }%DIFDELCMD < \eqref{eq:analytical-power-law}%%%
\DIFdel{) results in:
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eq:gradu-power-law}%%%
  \frac{\partial u_x}{\partial y} = \left( -\frac{1}{k} \pgrad \right)^{1/n} \left( -y |y|^{\frac{1}{n} - 1} \right).
}\end{displaymath}
%DIFAUXCMD
\DIFdel{It can be seen from }%DIFDELCMD < \eqref{eq:gradu-bingham} %%%
\DIFdel{and }%DIFDELCMD < \eqref{eq:gradu-power-law} %%%
\DIFdel{that $\left| \frac{\partial u_x}{\partial y} \right|$ increases with the yield stress for Bingham plastics, and increases with $\frac{1}{n}$ for power-law fluids, which suggests that one of the challenges associated with the accuracy and stability of simulating higher yield-stress fluids and more extreme shear-thinning fluids (e.g. $n \le 0.5$) in Poiseuille-type flow is a result of having a larger magnitude of the gradient of macroscopic flow.
This conclusion is consistent with the discussion in }%DIFDELCMD < \Fref{sec:bgk} %%%
\DIFdel{with regards to one of the drawbacks of the BGK collision operator, namely that a sharp spatial gradient in either $\rho$ or $\mvel$ can lead to nonphysical oscillations that can degrade the accuracy and stability of the solution.
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Lid-driven Cavity Flow} % MG: would it be of any use to show streamline plots from some of these flows?
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Unless specified otherwise, the }\DIFaddend lid-driven cavity simulations presented in the section were all simulated on a relatively coarse, $100 \times 100$ lattice, similar to the work presented in ~\cite{brownlee2008nonequilibrium}.
A coarse lattice was chosen in order to highlight concerns with stability and accuracy.
The simulations were run for either 50000 \DIFdelbegin \DIFdel{timesteps }\DIFdelend \DIFaddbegin \DIFadd{time steps }\DIFaddend or until convergence was met.
Convergence was defined by:
\begin{equation} \label{eq:convergence}
\sum_{m=1}^{100} \sum_{i, j} \frac{|u_{i, j}^k - u_{i, j}^{k-m}|}{|u_{i, j}^{k-m}|} < 1.0 \times 10^{-7},
\end{equation}
\noindent where $i$ is the node index in the x-direction, $j$ is the node index in the y-direction, and $k$ is the current \DIFdelbegin \DIFdel{timestep}\DIFdelend \DIFaddbegin \DIFadd{time step}\DIFaddend .
The lid velocity was prescribed as $u_o = 0.1$.
\DIFdelbegin \DIFdel{For the MRT relaxation matrix, the free parameters were set to $s_1 = 1.1$, $s_2 = 1.0$, and $s_4 = s_6 = 1.2$, as these values have been successfully applied to simulating lid-driven cavity flow of non-Newtonian fluids in the past~\mbox{%DIFAUXCMD
\cite{chen2014simulations,li2014simulation}}%DIFAUXCMD
.
}\DIFdelend All coordinate values presented in this section (used to describe the location of the center of vortices) are given normalized with respect to the length of the cavity side (i.e. as $(x / L, y / L)$).
...
\DIFaddbegin \DIFadd{For the MRT relaxation matrix, the free parameters were set to $s_1 = 1.1$, $s_2 = 1.0$, and $s_4 = s_6 = 1.2$ (same as MRT-2 in }\Fref{sec:poise-bing}\DIFadd{).
}\DIFaddend Tables \ref{tab:lid-bing1}--\ref{tab:lid-bing100} compare the center location of the main vortex to literature values taken from~\citet{syrakos2014performance}.
...
However, although the BGK with $m = 10^5$ was the fastest model in all cases, it was also \DIFdelbegin \DIFdel{similar to the other two }\DIFdelend \DIFaddbegin \DIFadd{unstable for $Re \ge 5000$.
...
With the finer lattice, the }\DIFaddend BGK models without median filtering \DIFdelbegin \DIFdel{in that it was unstable for $Re \ge 5000$}\DIFdelend \DIFaddbegin \DIFadd{were numerically unstable and took 85\% to 308\% of the computational time of the corresponding $100 \times 100$ lattice MRT model.
BGK tests conducted on a $300 \times 300$ lattice with $m = 10^8$ (without entropic filtering) for Bingham numbers of $\begin{bmatrix}1,&10\end{bmatrix}$ and Reynold’s numbers of $\begin{bmatrix}5000,&10000\end{bmatrix}$ also yielded unstable results}\DIFaddend .
...
%DIF <  MG: verify this sentence is correct
...
The \DIFdelbegin \DIFdel{extreme }\DIFdelend increase in computational expense is probably not due to the collision process itself (i.e. \eqref{eq:mrt-colop}) but instead due to calculating the strain-rate \eqref{eq:srtensor-mrt} for each iteration of the solution for the apparent viscosity, $\mu_{app}$.
...
In summary, for the lid-driven cavity flow of a Bingham plastic fluid, 
 \begin{itemize} 
    ...
    \item and the MRT collision operator with $m = 10^8$ produces solutions with high accuracy and stability for all of the cases examined herein, but at an increased (approximately \DIFdelbegin \DIFdel{5-10 }\DIFdelend \DIFaddbegin \DIFadd{5--10 }\DIFaddend times more) computing cost.
 \end{itemize}  

\subsubsection{Power-law Fluids}

For the power-law numerical tests the Reynold's number was varied with the following values: $\begin{bmatrix}100,&1000,&5000,&10000\end{bmatrix}$, and the flow behavior index, $n$, was varied with the following values: $\begin{bmatrix}0.5,&1.5\end{bmatrix}$\DIFdelbegin \DIFdel{($Re$ was calculated the same as in }%DIFDELCMD < \Fref{sec:poise-powerlaw}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{.
The Reynold's number was calculated as $\rho \frac{U^{2-n} H^n}{k}$}\DIFaddend .
...
However, the solutions became unstable at higher Reynold's numbers and entropic filtering was necessary to produce a stable \DIFdelbegin \DIFdel{soltuion }\DIFdelend \DIFaddbegin \DIFadd{solution }\DIFaddend when $Re \ge 1000$ for the BGK collision operator, and when $Re \ge 5000$ for the MRT collision operator.
...
The results showed that for higher yield stress fluids in Poiseuille-type flow, only the MRT collision operator was able \DIFaddbegin \DIFadd{to }\DIFaddend use an accurate approximation to true Bingham plastic behavior and not suffer from nonphysical oscillations.
...
For nearly all of the cases considered in the current study, the MRT collision operator was more accurate, but also more computationally expensive than its BGK counterpart, and is \DIFdelbegin \DIFdel{some times }\DIFdelend \DIFaddbegin \DIFadd{sometimes }\DIFaddend even orders of magnitude slower.

In regards to future work, \cite{conrad2015accuracy} showed that non-Newtonian Lattice Boltzmann simulations are not \DIFdelbegin \DIFdel{second order }\DIFdelend \DIFaddbegin \DIFadd{second-order }\DIFaddend accurate for every simulation Mach number, and showed that a greater accuracy may be achieved by properly tuning the simulation Mach number.
\cite{conrad2015accuracy} predicted optimal simulation Mach numbers for various shear-thinning and shear-thickening fluids. 
\DIFdelbegin \DIFdel{Possbile }\DIFdelend \DIFaddbegin \DIFadd{Possible }\DIFaddend future work would include extending the numerical investigation to consider the effect of the simulation Mach number on the accuracy, stability, and efficiency of the various collision operators and stability enhancements in simulating non-Newtonian flow.
 \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  MG: should we mention: code is open source? input files used in this study are also freely available?
%DIFDELCMD < 
